    ...
    def forward(self, x, inducing_points, inducing_values, variational_inducing_covar=None, **kwargs):
        ...
        if L.shape != induc_induc_covar.shape:
            ...
            try:
                ...
                pass
            L = self._cholesky_factor(induc_induc_covar)
        interp_term = L.solve(induc_data_covar.type(_linalg_dtype_cholesky.value())).double()#.to(full_inputs.dtype)

        # Compute the mean of q(f)
        # k_XZ K_ZZ^{-1/2} (m - K_ZZ^{-1/2} \mu_Z) + \mu_X
        predictive_mean = (interp_term.transpose(-1, -2) @ inducing_values.unsqueeze(-1).double()).squeeze(-1) + test_mean.double()

        # Compute the covariance of q(f)
        # K_XX + k_XZ K_ZZ^{-1/2} (S - I) K_ZZ^{-1/2} k_ZX
        middle_term = self.prior_distribution.lazy_covariance_matrix.mul(-1).double()
        if variational_inducing_covar is not None:
            middle_term = SumLinearOperator(variational_inducing_covar, middle_term).double()
        ...
        print('inside variational strategy', predictive_mean.shape,predictive_covar.shape)
    ...
    def __call__(self, x, prior=False, **kwargs):
        if not self.updated_strategy.item() and not prior:
            with torch.no_grad():
                ...
                print('inside variational strategy call', whitened_mean.shape,whitened_covar.shape)
